...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on('data-updated', (data: { senderId: string }) => {
      if (data.senderId !== clientId) {
        // Re-fetch data if update came from someone else
        fetchData();
      }
    });
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.yearBoundaries && Object.keys(data.yearBoundaries).length > 0) {
          // Check if we need to migrate to the new standards requested by the user
          const needsMigration = data.yearBoundaries[7]?.some((b: any) => b.grade === 'A*');
          if (needsMigration) {
            setYearBoundaries({
              7: [...KS3_BOUNDARIES],
              8: [...KS3_BOUNDARIES],
            ...
            setYearBoundaries(data.yearBoundaries);
          }
        }
      }
    } catch (error) {
  ...
  }, [students, assessments, marks, groups, yearBoundaries, isInitialLoading]);
  ...
  const handlePaperUpload = async (e: React.ChangeEvent<HTMLInputElement>, assessmentId: string) => {
    ...
    try {
      ...
      const prompt = extractionMode === 'questions' 
        ? "Extract all main question numbers (e.g., 1, 2, 3) and their total maximum marks for that number. If a question has sub-parts, sum them up for the main number. Return a JSON array of objects with 'number' (string) and 'maxMarks' (number) properties. Only return the JSON."
        : "Extract all question numbers including their sub-parts (e.g., 1a, 1b, 2a, 2b(i)) and their individual maximum marks. Each sub-part should be a separate entry. Return a JSON array of objects with 'number' (string) and 'maxMarks' (number) properties. Only return the JSON.";
      ...
      setAssessments(prev => prev.map(a => 
        a.id === assessmentId 
          ? { ...a, questions: extractedQuestions, maxMarks: extractedQuestions.reduce((sum, q) => sum + q.maxMarks, 0) } 
          : a
      ));
    } catch (err) {
      console.error("Extraction error:", err);
      setExtractionError("Failed to extract questions. Please try again or enter manually.");
    } finally {
