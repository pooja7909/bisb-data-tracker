...
async function startServer() {
  ...
  app.post("/api/save-all", (req, res) => {
    ...
    const transaction = db.transaction(() => {
      ...
      // Insert boundaries
      const insertBoundary = db.prepare("INSERT INTO boundaries (yearGroup, grade, minPercentage) VALUES (?, ?, ?)");
      Object.entries(yearBoundaries).forEach(([yearGroup, boundaries]: [string, any]) => {
        boundaries.forEach((b: any) => insertBoundary.run(yearGroup, b.grade, b.minPercentage));
      });
    });
